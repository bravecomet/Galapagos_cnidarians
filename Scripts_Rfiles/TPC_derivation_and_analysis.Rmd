...
write.csv(Pmax, "Coral_Pmax.csv")
```

#Outlier removal method IQR*1.5: Topt, E, Eh, Pmax per species
```{r}
# Your dataset with a single variable (e.g., "data")
#enter value set here, for each parameter per species, ex. for Pmax for B. grande below
Pmax_Bgran_data <- c(3.60994377, 3.67920447, 3.36060898, 3.06970286, 3.15170139, 3.59672995, 3.09289523)
...
while (TRUE) {
  ...
  mean_data <- mean(Pmax_Bgran_data)
  sd_data <- sd(Pmax_Bgran_data)

  # Calculate Z-scores for each data point
  z_scores <- (Pmax_Bgran_data - mean_data) / sd_data
  ...
  cat("Outliers:", Pmax_Bgran_data[outliers], "\n")
  all_outliers <- c(all_outliers, Pmax_Bgran_data[outliers])
  
  # Remove the first outlier found
  Pmax_Bgran_data <- Pmax_Bgran_data[-outliers[1]]
}

# Print all outliers
cat("All outliers:", all_outliers, "\n")

# Remove manually from dataset and repeat for each species/parameter pairing, updating each parameter .csv accordingly
```
...
# Read data
mydata <- read.csv('Photo.R_full.csv')
